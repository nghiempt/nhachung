  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const r = await api.signup({
        ...
        password,
        fullName,
        invitationCode: referral || undefined,
      });
      auth.setTokens(r.accessToken, r.refreshToken);
      router.replace("/dashboard");
    } catch (err: any) {
      setError(err?.body?.error?.message ?? "Tạo tài khoản thất bại. Vui lòng thử lại.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="login-page">
        ...
        <div className="form-wrap">
          ...
          <form className="login-form" onSubmit={onSubmit}>
            ...
            <div className="field">
              ...
              />
              <p className="field-hint">Bạn sẽ được giảm giá khi nâng cấp gói. Mã không hợp lệ sẽ bị bỏ qua.</p>
            </div>
            <div className="checkbox-row">
              ...
              <label htmlFor="agree">
                Tôi đồng ý với{" "}
                <a href="https://zalo.webico.ai/terms" target="_blank" rel="noreferrer">Điều khoản dịch vụ</a>
                {" "}và{" "}
                <a href="https://zalo.webico.ai/privacy" target="_blank" rel="noreferrer">Chính sách bảo mật</a>.
              </label>
            </div>
      ...
      <style jsx>{`
        ...
        .left-panel { flex: 1 1 0; min-width: 0; height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: clamp(16px, 4vh, 60px); padding: clamp(16px, 2.5vw, 48px) clamp(20px, 4vw, 60px); overflow-y: auto; }
        ...
        .field-hint { font-size: 12px; color: #666; line-height: 16px; }
        .checkbox-row { display: flex; align-items: flex-start; gap: 10px; width: 100%; }
        .checkbox-row :global(input[type="checkbox"]) { appearance: none; -webkit-appearance: none; width: 18px; height: 18px; min-width: 18px; border: 1px solid #D4D7E5; border-radius: 5px; background: #fff; cursor: pointer; transition: border-color .2s, background .2s; margin-top: 1px; }
        .checkbox-row :global(input[type="checkbox"]:checked) { background: #4137F9; border-color: #4137F9; background-image: url("data:image/svg+xml,%3Csvg width='11' height='8' viewBox='0 0 11 8' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 4L4 7L10 1' stroke='white' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E"); background-repeat: no-repeat; background-position: center; }
        .checkbox-row :global(label) { font-size: 13px; color: #666; line-height: 18px; cursor: pointer; }
        .checkbox-row :global(label a) { color: #4137F9; font-weight: 600; text-decoration: underline; }
        .checkbox-row :global(label a:hover) { opacity: .8; }
